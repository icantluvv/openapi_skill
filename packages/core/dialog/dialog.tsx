...
export function DialogContent({ children, className, ref, ...props }: DialogContentProps) {
    return (
        <DialogPortal>
            ...
            >
                {children}
                <DialogPrimitive.Close className="ring-offset-background focus:ring-ring focus:outline-hidden data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none">
                    <X className="size-4" />
